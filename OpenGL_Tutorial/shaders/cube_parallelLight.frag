...
void main(){
	// Мировое освещение:
	vec3 ambient = dirLight.ambient * vec3(texture(material.diffuse, fTextureCoord));

	// Диффузионный свет:
	vec3 norm = normalize(fNormal);  // Нормаль к поверхности
	vec3 lightDir = normalize(-dirLight.direction); // Вектор направления света
	...
	vec3 diffuse = dirLight.diffuse * diff * vec3(texture(material.diffuse, fTextureCoord));
