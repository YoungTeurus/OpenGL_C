...
private:
	...
	VOsAndIndices* generateForLineStrip()
	{
		std::vector<unsigned> lineIndicesData = StaticFigures::getLineIndices();
		
		// VAO для отрисовки шрифта с динамическим VBO
		unsigned lineVAO, lineVBO, lineEBO;
		...
		// Временно пустой VBO (2-ре вертекса по 3-ре float):
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 2 * 3, NULL, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lineEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned) * lineIndicesData.size(), lineIndicesData.data(), GL_STATIC_DRAW);
		...
		addInLoaded("LineStrip", generatedVOs);
...
public:
	...
	VOsAndIndices* getLineStrip()
	{
		VOsAndIndices *returnValue = getByName("LineStrip");
		if (returnValue == nullptr)
		{
			returnValue = generateForLineStrip();
		}
		return returnValue;
	}
};
